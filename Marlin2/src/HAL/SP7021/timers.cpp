 ...
 *
 */
#if 1//defined(ARDUINO_ARCH_STM32) && !defined(STM32GENERIC)
...
#define ADS1015_TIMER_DEV _TIMER_DEV(ADS1015_TIMER)
  ...
  if (!HAL_timer_initialized(timer_num)) {
    switch (timer_num) {
      case STEP_TIMER_NUM: // STEPPER TIMER - use a 32bit timer if possible
		...
		timer_instance[timer_num]->setCount(_MIN(hal_timer_t(HAL_TIMER_TYPE_MAX), (HAL_TIMER_RATE) / (STEPPER_TIMER_PRESCALE) /* /frequency */), TICK_FORMAT);
		break;
...
static constexpr struct {uintptr_t base_address; int timer_number;} sp7021_timer_map[] = {

  #ifdef TIM7_BASE
    { uintptr_t(TIM7_BASE),   7 },
  #endif
  #ifdef TIM6_BASE
    { uintptr_t(TIM6_BASE),   6 },
  #endif
  #ifdef TIM5_BASE
    { uintptr_t(TIM5_BASE),   5 },
  #endif
  #ifdef TIM4_BASE
    { uintptr_t(TIM4_BASE),   4 },
  #endif
  #ifdef TIM3_BASE
    { uintptr_t(TIM3_BASE),   3 },
  #endif
  #ifdef TIM2_BASE
    { uintptr_t(TIM2_BASE),   2 },
  #endif
  #ifdef TIM1_BASE
    { uintptr_t(TIM1_BASE),   1 },
  #endif
  #ifdef TIM0_BASE
    { uintptr_t(TIM0_BASE),   0 }
  #endif
};
...
static constexpr int get_timer_num_from_base_address(uintptr_t base_address) {
  for (const auto &timer : sp7021_timer_map)
    if (timer.base_address == base_address) return timer.timer_number;
  return 0;
}

// The platform's SoftwareSerial.cpp will use the first timer from sp7021_timer_map.
#if HAS_TMC_SW_SERIAL && !defined(TIMER_SERIAL)
  #define  TIMER_SERIAL (sp7021_timer_map[0].base_address)
#endif
