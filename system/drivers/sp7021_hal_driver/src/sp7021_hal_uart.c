...
static void __uart_tx_irq_handler(UART_HandleTypeDef *huart)
{
	...
	uart_assert_param(huart->txdma);
	uart_assert_param(huart->txgdma);
...
{
	uart_assert_param(huart->pRxBuffPtr);
